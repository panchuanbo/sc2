...
class ActorNetwork(object):
	...
	def __init__(self, sess, learning_rate, tau):
		...
		self.action_gradient = tf.placeholder(tf.float32, [None, 3, 3])
	...
	def model(self):
		...
		return state, is_training, tf.reshape(tf.layers.dense(out, 9), [-1, 3, 3])
...
class CriticNetwork(object):
	...
	def model(self):
		state = tf.placeholder(tf.float32, [None, 84, 84, 17])
		action = tf.placeholder(tf.float32, [None, 3, 3])
		is_training = tf.placeholder(tf.bool)
