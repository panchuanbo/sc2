...
class ActorNetwork(object):
	...
	def model(self):
		...
		out = tf.layers.conv2d(state, 32, 5, 1, "SAME")#, kernel_regularizer=regularizer)
		out = tf.nn.relu(out)
		for i in range(3):
...
def load_data(mini = False):
	...
	if mini == True:
		...
		X = np.append(np.load("Features1.npy"), np.load("Features2.npy"), axis=0)
		y = np.append(np.load("Actions1.npy"), np.load("Actions2.npy"), axis=0)
