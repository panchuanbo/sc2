...
class DeepQAgent(base_agent.BaseAgent):
    ...
    def __init__(self):
        ...
        self.network = DeepQNetwork(self.sess, ACTOR_LEARNING_RATE, TAU)
    ...
    def step(self, obs):
        use_location_heuristic = False # Instead of manually find where the agent last was, use the last action location as a heuristic
        recompute_action_from_location = False # doesn't use self.lastnetworklocation if true
        global EPSILON
        ...
        if not self.laststate is None:
            network_action = self.sc2tonetwork(self.lastaction[0], self.lastaction[1]) if recompute_action_from_location else self.lastnetworkaction
            self.buffer.add(
                self.laststate,          # S
                network_action,  # A
                self.lastreward,         # R
        ...
        if use_location_heuristic == True:
            if not self.laststate is None:
